...
class MaryLSTMModel(nn.Module):
    def __init__(self, hidden_dim=512, embedding_dim=64, dropout_rate=0.4, use_batch_norm=False):
        ...
        if use_batch_norm:
            self.bn1 = nn.BatchNorm1d(hidden_dim * 2 + 26 + 5 + 2)  # *2 for bidirectional, +2 for vowel ratio and remaining lives
        ...
        # Final dense layers
        self.combine = nn.Linear(hidden_dim * 2 + 26 + 5 + 2, hidden_dim)  # *2 for bidirectional
    ...
    def forward(self, word_state, guessed_letters, word_length, vowel_ratio, remaining_lives):
        ...
        lstm_out, _ = self.lstm(embedded)  # [batch_size, seq_len, hidden_size*2] (*2 for bidirectional)
        
        # Get final LSTM output (concatenate forward and backward)
...
def generate_dataset(words, ngram_dict, is_validation=False, train_words=None):
    ...
    with Pool() as pool:
        ...
        word_pairs = [(word, is_validation) for word in words for _ in range(2)]
        all_states = list(tqdm(
            pool.imap(simulate_game_states, words, chunksize=10),
            total=len(words) * 2,  # Double the progress bar total
...
def setup_logging():
    ...
    log_file = log_dir / f'mary_hangman_nv4_{timestamp}.log'
...
def train_model(model, train_states, val_states, val_words, epochs=EPOCHS):
    ...
    def save_checkpoint(epoch, val_loss):
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        path = f'{DATA_DIR}/mary_model_nv4_epoch{epoch}_{timestamp}.pt'
...
def validate_states(states, num_samples=5):
    ...
    for i, state in enumerate(sample_states):
        ...
        try:
            ...
            # Test concatenation with doubled hidden size for bidirectional
            mock_lstm_out = torch.zeros((1, hidden_dim * 2))  # *2 for bidirectional
            combined = torch.cat([
                mock_lstm_out,  # [1, hidden_dim*2]
                guessed,  # [1, 26]
                length_encoding,  # [1, 5]
                vowel_ratio,  # [1, 1]
                lives  # [1, 1]
    ...
    try:
        ...
        if args.latest_weights or args.load_weights:
            if args.latest_weights:
                ...
                model_files = [f for f in os.listdir(DATA_DIR) if f.startswith('mary_model_nv4_') and f.endswith('.pt')]
                if not model_files:
        ...
        # Save results
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_path = f'{DATA_DIR}/mary_model_nv4_{timestamp}.pt'
        metrics_path = f'{DATA_DIR}/mary_metrics_nv4_{timestamp}.pkl'
