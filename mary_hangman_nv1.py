...
def save_data(train_states, val_states, val_words):
    ...
    save_path = data_dir / f'hangman_states_nv1_{timestamp}.pkl'
...
def load_data(force_new=False):
    ...
    dataset_files = list(data_dir.glob('hangman_states_nv1_*.pkl'))
...
def setup_logging():
    ...
    log_file = log_dir / f'mary_hangman_nv1_{timestamp}.log'
...
def train_model(model, train_states, val_states, val_words, epochs=EPOCHS):
    ...
    def save_checkpoint(epoch, val_loss):
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        path = f'{DATA_DIR}/mary_model_nv1_epoch{epoch}_{timestamp}.pt'
...
def main():
    ...
    try:
        ...
        # Save results
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_path = f'{DATA_DIR}/mary_model_nv1_{timestamp}.pt'
        metrics_path = f'{DATA_DIR}/mary_metrics_nv1_{timestamp}.pkl'
