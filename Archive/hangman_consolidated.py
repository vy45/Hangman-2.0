...
def train_model(model, train_states, val_states, val_words, model_type, epochs=10):
    ...
    try:
        ...
        # # TEMPORARY: Break after 100 batches for testing
        # test_mode = True
        ...
        for epoch in range(epochs):
            ...
            for i in batch_iterator:
                # if test_mode and num_valid_batches >= max_test_batches:
                #     print("\nBreaking after 100 batches (test mode)")
                #     break
            ...
            # if test_mode and num_valid_batches >= max_test_batches:
            #     print("\nQuick test mode: Completed 100 batches")
            #     break
                
            # Validation phase
            val_pbar = tqdm(desc="Validation", position=1, leave=False)
            eval_start_time = time.time()
            val_loss = evaluate_model(model, val_states, model_type)
            ...
            completion_rate = calculate_completion_rate(model, val_words[:COMPLETION_EVAL_WORDS], model_type)
            comp_pbar.close()
            ...
            # # Test on specific word every other epoch
            # if epoch % 2 == 0:
            #     test_predictions_on_word(model, model_type, "congratulations")
...
def evaluate_model(model, val_states, model_type):
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(0, len(val_states), BATCH_SIZE), 
                     desc="Validation", 
                     leave=False):
...
if __name__ == "__main__":
    ...
    try:
        # Only train Transformer (1) and MLP (4)
        models_to_train = [5, 2, 3, 1, 4]
